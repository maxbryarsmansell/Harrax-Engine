...
import com.max.harrax.graphicsOLD.text.Glyph;
...
public class Renderer {
	...
	public Renderer() {
		init(new Shader("/shaders/batch.vert", "/shaders/batch.frag"), new Camera(800, 600));
	}
	...
	public void submitQuad(Renderable renderable) {
		...
		Vec4 position = renderable.getPosition().toVec4().Multiply(transformationStack.lastElement());
	...
	public void submitText(Font font, float x, float y, Colour colour, CharSequence text, float scale) {
		// Get the font width and height
		int fontWidth = font.getFontWidth();
		int fontHeight = font.getFontHeight();
		
		float scaledFontWidth = fontWidth * scale;
		float scaledFontHeight = fontHeight * scale;
		...
		for (int i = 0; i < text.length(); i++) {
			...
			// Convert the position and size to float coordinates
			float x0 = (position.x);
			float x1 = (position.x + scaledWidth);
			float y0 = (position.y);
			float y1 = (position.y + scaledHeight);
	...
	public void drawQuad(float x, float y, float width, float height, Colour colour) {
		// check whether there is enough space in the current batch
		if (vertices.remaining() < RENDERABLE_VERTEX_COUNT * VERTEX_SIZE) {
			flush();
		}

		Vec4 position = new Vec4(x, y, 0, 1).Multiply(transformationStack.lastElement());
